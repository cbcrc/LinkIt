...
namespace LinkIt.ReferenceTrees {
    public class ReferenceDependencyDag {
        readonly Dictionary<Type, ReferenceDependencyDagNode> _nodeByReferenceType = new Dictionary<Type, ReferenceDependencyDagNode>();
        ...
        public void AddDependency(Type from, Type to) {
            if (GetNode(to) == null) {
                throw new ArgumentException("Cannot add dependency to reference type not yet in the graph", nameof(to));
            }
        ...
        public List<List<Type>> ReferenceTypeToBeLoadedForEachLoadingLevel() {
            return _nodeByReferenceType.Values
                .GroupBy(
                    keySelector: p => p.LongestDependecyChainLength, //loadingLevel
                    elementSelector: p => p.ReferenceType
                )
