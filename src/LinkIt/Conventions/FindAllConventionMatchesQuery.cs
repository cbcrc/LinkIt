        ...
        private static bool DoesConventionApply(ConventionMatch match)
        {
            ...
            catch (Exception ex)
            {
                throw new LinkItException(
                    $"The convention \"{match.Convention.Name}\" failed for DoesApply. Link target id: {match.LinkTargetProperty.GetFriendlyName()}, linked source model property: {match.LinkedSourceModelProperty.GetFriendlyName()}",
                    ex
                );
            }
        }
